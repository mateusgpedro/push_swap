...
void find_lowest(stack_t *stackA, stack_t *stackB, int size)
{
    int *first;
    int *last;

    first = subdivide_first_elements(stackA);
    last = subdivide_last_elements(stackA, size);
    if (*first > *last)
...
void rotate_to_lowest(t_stack *stackA, stack_t *stackB, int dir, int value) {
